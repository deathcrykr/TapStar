		...
		public void DrawUI()
		{
			...
			using (new GUILayout.VerticalScope(GUI.skin.box))
			{
				...
				GUILayout.Label($"Running: {SpeedHackDetector.Instance.IsRunning}\n" +
								ExamplesGUI.ColorizeGreenOrRed($"Detected: {speedHackDetected.ToString().ToLower()}", !speedHackDetected));
				...
				if (wrongTimeDetected && !timeCheatingDetected)
				{
					GUILayout.Label($"Running: {TimeCheatingDetector.Instance.IsRunning}\n" +
									ExamplesGUI.Colorize(
										"Wrong time detected (diff more than: {TimeCheatingDetector.Instance.wrongTimeThreshold} minutes)",
				...
				GUILayout.Label($"Running: {ObscuredCheatingDetector.Instance.IsRunning}\n" + 
								ExamplesGUI.ColorizeGreenOrRed($"Detected: {obscuredTypeCheatDetected.ToString()}", !obscuredTypeCheatDetected));
				...
				GUILayout.Label($"Running: {WallHackDetector.Instance.IsRunning}\n" + 
								ExamplesGUI.ColorizeGreenOrRed($"Detected: {wallHackCheatDetected.ToString()}", !wallHackCheatDetected));

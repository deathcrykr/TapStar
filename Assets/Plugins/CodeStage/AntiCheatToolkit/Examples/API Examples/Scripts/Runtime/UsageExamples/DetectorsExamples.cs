...
namespace CodeStage.AntiCheat.Examples
{
	...
	internal partial class DetectorsExamples : MonoBehaviour
	{
		...
		public void OnObscuredTypeCheatingDetected()
		{
			...
			// you can get access to the latest detection information and find out more about the detection context
			var detection = ObscuredCheatingDetector.Instance.LastDetectionInfo;
		...
		private void OnTimeCheatChecked(TimeCheatingDetector.CheckResult checkResult,
		...
		private void SpeedHackDetectorExample()
		{
			...
			// --------------------------------------------
			/* TimeScale detection examples (new in 6.2.0) */
			// --------------------------------------------

			// SpeedHackDetector now also detects unauthorized Time.timeScale changes.
			// Use the safe API to change timeScale without triggering detection:

			// Safe timeScale changes - won't trigger detection:
			// SpeedHackDetector.SetTimeScale(0.5f);  // Slow motion
			// SpeedHackDetector.SetTimeScale(0f);    // Pause
			// SpeedHackDetector.SetTimeScale(1f);    // Normal speed

			// For third-party assets that need temporary timeScale control:
			// SpeedHackDetector.AllowAnyTimeScaleFor(3f);  // Allow for 3 seconds
			// Time.timeScale = 0.5f; // Third-party asset can change this freely

			// For cutscene systems that need full control:
			// SpeedHackDetector.AllowAnyTimeScale();       // Allow indefinitely
			// Time.timeScale = 0f; // Timeline/cutscene system controls this
			// SpeedHackDetector.StopAllowingAnyTimeScale(); // Resume detection when done

			// Drop-in replacement for Time.timeScale:
			// SpeedHackProofTime.timeScale = 2f; // Uses safe API internally
		}
		...
		internal void ForceTimeCheatingDetectorCheck()
		{
			...
			if (TimeCheatingDetector.Instance != null && 
				TimeCheatingDetector.Instance.IsReadyForForceCheck())
			{
				TimeCheatingDetector.Instance.ForceCheck();
			}
#endif

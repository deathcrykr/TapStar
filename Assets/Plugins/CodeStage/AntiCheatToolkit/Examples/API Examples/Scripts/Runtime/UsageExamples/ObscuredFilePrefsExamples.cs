...
namespace CodeStage.AntiCheat.Examples
{
	...
	internal partial class ObscuredFilePrefsExamples : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			/*	Note: Hardcoded strings in code are not secure - they're visible when decompiled.
				Better approaches:
				- Generate keys from user credentials at runtime
				- Use server-provided session tokens
				- Combine device-specific data with user input, etc. 
				Important: The same key is needed to decrypt existing data. 
				Changing passwords makes old data unreadable. */

...
{
	/// <summary>
	/// Use it instead of regular <c>byte</c> for any cheating-sensitive properties, fields and other long-term declarations.
	/// </summary>
	...
	/// Use regular type for all short-term operations and calculations while keeping obscured type only at the long-term declaration (i.e. class field).
 	[Serializable]
	public partial struct ObscuredByte : IObscuredType
	{
		...
		[NonSerialized] private byte fakeValue;
		...
		{
			if (IsDefault()) this = new ObscuredByte(default);
		...
		private void HideValue(byte plain)
		{
		...
		/// </summary>
		public static byte DecryptFromV0(byte value, byte key)
		{

...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class SpeedHackDetector : ACTkDetectorBase<SpeedHackDetector>
	{
		...
		/// <strong>\htmlonly<font color="FF4040">WARNING:</font>\endhtmlonly Use at your peril!</strong>
		...
		/// <strong>\htmlonly<font color="FF4040">WARNING:</font>\endhtmlonly May cause false positives if you change timeScale
		/// without using the provided API.</strong>
		...
		/// <returns>True if timeScale was successfully changed, false if detector is not running or change was rejected.</returns>
		public static bool SetTimeScale(float newTimeScale)
		{
			...
			return Instance.SetTimeScaleInternal(newTimeScale);
		}
		...
		private void Update()
		{
			...
			if (cheatedEnvironment || cheatedRealtime || cheatedDsp || cheatedTimeScale)
			{
		...
		private bool SetTimeScaleInternal(float newTimeScale)
		{
			if (!WatchTimeScale)
			{
				Time.timeScale = newTimeScale;

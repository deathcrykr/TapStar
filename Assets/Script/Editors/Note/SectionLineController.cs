...
namespace TapStar.Editors
{
	...
	public class SectionLineController : MonoBehaviour
	{
		...
		public void Initialize(VocalSection section, GameManager manager, Canvas canvas, Transform center)
		{
			// Null 체크
			if (section == null)
			{
			...
			{
#if UNITY_EDITOR
				Debug.LogError("SectionLineController: GameManager is null!");
#endif
				return;
			}
...
#if UNITY_EDITOR
			Debug.Log($"🎤 Section line spawned: {section.Type} ({section.StartTime}s - {section.EndTime}s), width: {m_InitialWidth}→{m_TargetWidth}");
#endif
		}
		...
		public bool IsExpired()
		{
			...
			return m_GameManager.currentTime > m_SectionData.EndTime + EXPIRE_BUFFER_TIME;
		}
		...
		{
			return m_SectionData?.StartTime ?? 0f;
		}
		...
		{
			return m_SectionData?.EndTime ?? 0f;
		}
		...
		{
			// 애니메이션 시간 계산
			float timeToTarget = m_SectionData.StartTime - m_GameManager.currentTime;
		...
		private void StartSectionAnimation()
		{
			...
			// 구역 지속 시간 계산
			float sectionDuration = m_SectionData.EndTime - m_SectionData.StartTime;

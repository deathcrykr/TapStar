...
public class LineNoteController : MonoBehaviour
{
	...
	[Tooltip("게임 매니저 참조")]
	[SerializeField] private RhythmGameManager m_GameManager;
	public RhythmGameManager GameManager { get => m_GameManager; set => m_GameManager = value; }
	...
	public void Initialize(Note note, RhythmGameManager manager, Canvas canvas, Transform center)
	{
		...
		float timeToTarget = m_NoteData.time_seconds - m_GameManager.currentTime;

		// 타이밍 안전성 검증
		if (timeToTarget <= 0)
		{
			Debug.LogWarning($"⚠️ Note spawn timing issue: timeToTarget={timeToTarget}, noteTime={m_NoteData.time_seconds}, currentTime={m_GameManager.currentTime}");
			...
			m_RectTransform.DOAnchorPos(new Vector2(m_TargetX, targetY), timeToTarget).SetEase(Ease.Linear)
				.OnComplete(() =>
				{
					...
					DOVirtual.DelayedCall(0.2f, () =>
					{
						...
						{
							m_LineImage.DOFade(0f, 0.3f).SetEase(Ease.OutQuart)
								.OnComplete(() =>
								{
		...
		Debug.Log($"🎯 NOTE TIMING: Will reach center at {m_NoteData.time_seconds}s (current: {m_GameManager.currentTime}s, offset: {m_GameManager.AudioLatencyOffset}s)");
	...
	private void Update()
	{
		...
		float targetTime = m_NoteData.time_seconds - m_GameManager.currentTime;
	...
	{
		return m_NoteData.time_seconds - currentTime;
	}
